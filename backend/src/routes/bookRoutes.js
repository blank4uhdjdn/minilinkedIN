  ...
  import Book from "../models/Posts.js";
  ...
  try {
    ...
    res.json(posts);  // return array of all posts
  } catch (error) {
  ...
  try {
    const userPosts = await Book.find({ user: req.user._id })
      .sort({ createdAt: -1 }); // newest first

    res.json(userPosts); // return all posts by this user
  } catch (error) {
